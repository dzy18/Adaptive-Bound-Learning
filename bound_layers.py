...
import numpy as np
from torch.nn import Sequential, Conv2d, Linear, ReLU, PReLU, LeakyReLU
...
class BoundLeakyReLU(LeakyReLU):
    def __init__(self, bound_opts=None):
        super(BoundLeakyReLU, self).__init__()
        self.bound_opts = bound_opts
    
    @staticmethod
    def convert(bound_opts=None):
        l = BoundLeakyReLU(bound_opts)
        return l
    ...
    def interval_propagate(self, norm, h_U, h_L, eps):
        ...
        self.lower_l = h_L 
        tightness_loss = self.unstab.sum()
        return norm, F.leaky_relu(h_U), F.leaky_relu(h_L), tightness_loss, tightness_loss, \
               (h_U < 0).sum(), (h_L > 0).sum()
    ...
    def bound_backward(self, last_uA, last_lA):
        ...
        ub_r = self.upper_u.clamp(min=0)
        ub_r = torch.max(ub_r, lb_r + 1e-8)
        upper_d = (ub_r - lb_r * 0.01) / (ub_r - lb_r)
        upper_b = ub_r - ub_r * upper_d
        upper_d = upper_d.unsqueeze(1)
        lower_d = upper_d
        uA = lA = None
        ubias = lbias = 0
...
class BoundPReLU(PReLU):
    def __init__(self, bound_opts=None):
        super(BoundPReLU, self).__init__()
        self.bound_opts = bound_opts
    
    @staticmethod
    def convert(l, bound_opts=None):
        nl = BoundPReLU(bound_opts)
        nl.weight.data.copy_(l.weight.data)
        logger.debug(nl.weight.size())
        return nl
    ...
    def interval_propagate(self, norm, h_U, h_L, eps):
        ...
        #return norm, F.relu(h_U), F.relu(h_L), tightness_loss, tightness_loss, \
        #       (h_U < 0).sum(), (h_L > 0).sum()
        ub = torch.max(F.prelu(h_U, self.weight), F.prelu(h_L, self.weight)) 
        #lb = torch.min(torch.zeros(h_L.shape), F.prelu(h_L, self.weight)) if h_U >= 0 and h_L <= 0 \
                #else torch.min(F.prelu(h_U, self.weight), F.prelu(h_L, self.weight))
        lb_1 = torch.min(torch.zeros(h_L.shape).cuda(), F.prelu(h_L, self.weight))
        lb_2 = torch.min(F.prelu(h_U, self.weight), F.prelu(h_L, self.weight))
        pos_t = ((h_U >= 0).float()) * ((h_L <= 0).float())
        lb = pos_t * lb_1 + (1 - pos_t) * lb_2
    
        return norm, ub, lb, tightness_loss, tightness_loss, \
                (h_U < 0).sum(), (h_L > 0).sum()
    ...
    def bound_backward(self, last_uA, last_lA):
        ...
        ub_r = self.upper_u.clamp(min=0)
        ub_r = torch.max(ub_r, lb_r + 1e-8)
        if self.weight.sum() > 1.0:
            print("self weight sum: {}".format(self.weight.sum()))

        if self.bound_opts.get("same-slope", False):
            upper_d = (ub_r - lb_r * self.weight) / (ub_r - lb_r)
            upper_b = ub_r - ub_r * upper_d
            upper_d = upper_d.unsqueeze(1)
            lower_d = upper_d

            pos_t = (upper_d > 1.0).float().reshape(upper_b.shape)

            lower_b = upper_b * pos_t
            upper_b = upper_b * (1 - pos_t)

        else:
            if self.weight.sum() > 1.0:
                lower_d = (ub_r - lb_r * self.weight) / (ub_r - lb_r)
                lower_b = ub_r - ub_r * lower_d
                lower_d = lower_d.unsqueeze(1)

                pos_t = (ub_r.abs() >= lb_r.abs()).float().reshape(lower_d.shape)
                upper_d = pos_t * 1.0 + (1 - pos_t) * self.weight.sum()
                upper_b = torch.zeros(lower_b.shape).cuda()
            else:
                upper_d = (ub_r - lb_r * self.weight) / (ub_r - lb_r)
                upper_b = ub_r - ub_r * upper_d
                upper_d = upper_d.unsqueeze(1)

                pos_t = (ub_r.abs() >= lb_r.abs()).float().reshape(upper_d.shape)
                lower_d = pos_t * 1.0 + (1 - pos_t) * self.weight.sum()
                lower_b = torch.zeros(upper_b.shape).cuda()
        ...
        if last_uA is not None:
            ...
            mult_uA = pos_uA.view(last_uA.size(0), last_uA.size(1), -1)
            ubias = mult_uA.matmul(upper_b.view(upper_b.size(0), -1, 1)).squeeze(-1)
            mult_uA = neg_uA.view(last_uA.size(0), last_uA.size(1), -1)
            ubias += mult_uA.matmul(lower_b.view(lower_b.size(0), -1, 1)).squeeze(-1)
        ...
        if last_lA is not None:
            ...
            mult_lA = neg_lA.view(last_lA.size(0), last_lA.size(1), -1)
            lbias = mult_lA.matmul(upper_b.view(upper_b.size(0), -1, 1)).squeeze(-1)
            mult_lA = pos_lA.view(last_lA.size(0), last_lA.size(1), -1)
            lbias += mult_lA.matmul(lower_b.view(lower_b.size(0), -1, 1)).squeeze(-1)
        return uA, ubias, lA, lbias
...
class BoundSequential(Sequential):
    ...
    def convert(sequential_model, bound_opts=None):
        ...
        for l in seq_model:
            ...
            if isinstance(l, PReLU):
                layers.append(BoundPReLU.convert(l, bound_opts))
            if isinstance(l, LeakyReLU):
                layers.append(BoundLeakyReLU.convert(bound_opts))
    ...
    def full_backward_range(self, norm=np.inf, x_U=None, x_L=None, eps=None, C=None, upper=True, lower=True):
        ...
        for i, module in enumerate(modules):
            ...
            if isinstance(module, BoundReLU) or isinstance(module, BoundPReLU):
                last_module = i
                break
        
        for i in range(last_module + 1, len(modules)):
            if isinstance(modules[i], BoundReLU) or isinstance(modules[i], BoundPReLU):
...
class BoundDataParallel(nn.DataParallel):
