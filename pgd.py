...
def _pgd(config, model, X, y, epsilon, alpha, niters=100, step_size=0.01):
    ...
    for i in range(niters):
        ...
        # opt.zero_grad()
        loss = nn.CrossEntropyLoss()(output(config, model, X_pgd, alpha), y)

        # loss = -(torch.gather(output(config, model, X_pgd, alpha), 1, y_y)).sum()
        # loss = output(model,X_pgd,alpha)
        loss.backward()
        eta = step_size * X_pgd.grad.data.sign()
        eta = X_pgd.data + eta - X.data
        # print("before:", torch.max(abs(eta[1])))
        eta = torch.clamp(eta, -epsilon, epsilon)
        # print("after:", torch.max(abs(eta[1])))
        X_pgd.data = X.data + eta
        X_pgd.data = torch.clamp(X_pgd.data, 0.0, 1.0)
    # exit()
    err_pgd = (output(config, model, X_pgd, alpha).data.max(1)
               [1] != y.data).float().sum()
    return err, err_pgd


def pgd(config, loader, model, epsilon, alpha, verbose=False,
        robust=False):
...
def attack(config, loader, model, epsilon, alpha, verbose=False, atk=None,
    ...
    for i, (X, y) in enumerate(loader):
        X, y = Variable(X.cuda(), requires_grad=True), Variable(
            y.cuda().long())
        ...
        if verbose:
            print('clean err: {} | PGD err: {}'.format(err, err_pgd))

    err_rate = err_count / total_count
    pgd_err_rate = pgd_err_count / total_count
    print('[TOTAL] clean err: {:6.4f} | PGD err: {:6.4f}'.format(
        err_rate, pgd_err_rate))
